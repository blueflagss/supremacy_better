...
void HVH::update_manual_direction( ) {
	bool states[ 3 ] = {
			g_input.GetKeyPress( g_menu.main.antiaim.manual_aa_left.get( ) ),
			g_input.GetKeyPress( g_menu.main.antiaim.manual_aa_right.get( ) ),
			g_input.GetKeyPress( g_menu.main.antiaim.manual_aa_back.get( ) )
	};

	using_manual_direction = manual_dir != manual_direction::none;

	if ( !g_menu.main.antiaim.manual_aa.get( ) ) {
		using_manual_direction = false;
		return;
	}
...
void HVH::GetAntiAimDirection( ) {
	if ( g_menu.main.antiaim.manual_aa.get( ) ) {
		if ( using_manual_direction ) {
			if ( manual_dir == manual_direction::back )
				m_direction = g_cl.m_cmd->m_view_angles.y + 180.0f;

			else if ( manual_dir == manual_direction::left )
				m_direction = g_cl.m_cmd->m_view_angles.y + 90.0f;
