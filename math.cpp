...
void math::AngleMatrix( const ang_t& ang, const vec3_t& pos, matrix3x4_t& out ) {
	...
	SinCos( deg_to_rad( ang[ 1 ] ), &sy, &cy );
	SinCos( deg_to_rad( ang[ 0 ] ), &sp, &cp );
	SinCos( deg_to_rad( ang[ 2 ] ), &sr, &cr );
...
void math::VectorRotate( const vec3_t& in1, matrix3x4_t in2, vec3_t& out ) {
	float x = in1.dot( in2[ 0 ] );
	float y = in1.dot( in2[ 1 ] );
	float z = in1.dot( in2[ 2 ] );
