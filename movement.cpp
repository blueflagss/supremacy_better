...
void Movement::FastStop( ) {
	...
	const auto pressing_move = ( g_cl.m_cmd->m_buttons & ( IN_LEFT ) || g_cl.m_cmd->m_buttons & ( IN_FORWARD ) || g_cl.m_cmd->m_buttons & ( IN_BACK ) ||
		g_cl.m_cmd->m_buttons & ( IN_RIGHT ) || g_cl.m_cmd->m_buttons & ( IN_MOVELEFT ) || g_cl.m_cmd->m_buttons & ( IN_MOVERIGHT ) ||
		g_cl.m_cmd->m_buttons & ( IN_JUMP ) );

	if ( !pressing_move && g_menu.main.movement.faststop.get( ) && g_cl.m_speed > 15.f ) {
