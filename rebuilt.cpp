...
void rebuilt::SetSequence( CCSGOPlayerAnimState* state, int layer_idx, int sequence ) {
	const auto player = state->m_pPlayer;

	if ( !player || !state )
		return;

	static auto CCSGOPlayerAnimState__UpdateLayerOrderPreset = pattern::find( g_csgo.m_client_dll, XOR( "55 8B EC 51 53 56 57 8B F9 83 7F 60 00 0F 84 ? ? ? ? 83" ) ).as< void( __thiscall* )( CCSGOPlayerAnimState*, int, int ) >( );
...
int rebuilt::SelectWeightedSequence( CCSGOPlayerAnimState* state, int act ) {
	int seq = -1;
...
void rebuilt::SetCycle( CCSGOPlayerAnimState* state, int layer_idx, float cycle ) {
...
void rebuilt::SetWeightDeltaRate( CCSGOPlayerAnimState* state, int layer_idx, float old_weight ) {
...
void rebuilt::IncrementLayerCycleWeightRateGeneric( CCSGOPlayerAnimState* state, int layer_idx ) {
...
// ty ses ^-^
void rebuilt::Update( CCSGOPlayerAnimState* animstate, const ang_t& angles, int tick ) {
	...
	const CCSGOPlayerAnimState backup_animstate = *animstate;
	...
	*/
	...
	/* update animation */
	animstate->m_pPlayer->UpdateAnimState( animstate, angles );
...
void rebuilt::SetWeight( CCSGOPlayerAnimState* state, int layer_idx, float weight ) {
	auto layer = state->m_pPlayer->m_AnimOverlay( ) + layer_idx;
...
float rebuilt::GetLayerIdealWeightFromSequenceCycle( CCSGOPlayerAnimState* state, int layer_idx ) {
	static auto CCSGOPlayerAnimState__GetLayerIdealWeightFromSeqCycle = pattern::find( g_csgo.m_client_dll, XOR( "55 8B EC 83 EC 08 53 56 8B 35 ? ? ? ? 57 8B F9 8B CE 8B 06 FF 90" ) ).as< float( __thiscall* )( CCSGOPlayerAnimState*, int ) >( );
	return CCSGOPlayerAnimState__GetLayerIdealWeightFromSeqCycle( state, layer_idx );
}

bool rebuilt::IsLayerSequenceCompleted( CCSGOPlayerAnimState* state, int layer_idx ) {
	const auto layer = state->m_pPlayer->m_AnimOverlay( ) + layer_idx;
...
void rebuilt::IncrementLayerCycle( CCSGOPlayerAnimState* state, int layer_idx, bool allow_loop ) {
	const auto layer = state->m_pPlayer->m_AnimOverlay( ) + layer_idx;
...
int rebuilt::GetLayerActivity( CCSGOPlayerAnimState* state, AnimationLayer_t layer_idx ) {
	static auto CCSGOPlayerAnimState__GetLayerActivity = pattern::find( g_csgo.m_client_dll, XOR( "51 53 56 8B 35 ? ? ? ? 57 8B F9 8B CE 8B 06 FF 90 ? ? ? ? 8B 7F 60 83 BF" ) ).as< int( __thiscall* )( CCSGOPlayerAnimState*, int ) >( );
...
void rebuilt::CalculatePoses( CCSGOPlayerAnimState* state, Player* player, float* poses, float feet_yaw ) {
	auto state_game = ( CCSGOPlayerAnimState* )state;
